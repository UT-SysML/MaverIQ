...
def parse_arguments(args=None):
    ...
    parser.add_argument('--run_as_server',
                        default=False,
                        action='store_true',
                        help="Whether or not to execute in an interactive mode")
...
def run_as_server(args, runner, tokenizer, pad_id, end_id, bad_words_list = None, stop_words_list = None, prompt_template = None):
    try:
        i = 0
        comm = MPI.COMM_WORLD
        ...
        while True:
            ...
            if rank==0:
                user_input = input("Enter input text or a list (use [a, b, c] format)(Ctrl+C to exit): ")
                print(f"You entered: {user_input}")
                i += 1
                print(f'inference task #{i}')
            
                try:
                    inputs = ast.literal_eval(user_input)
            ...
            if rank==0:
                ...
                print(
                    f"batch_size: {len(batch_input_ids)}, avg latency: {tensorrt_llm.profiler.elapsed_time_in_sec('tmp')} sec"
                )
...
def main(args):
    ...
    if args.run_as_server:
        run_as_server(args, runner, tokenizer, pad_id, end_id, bad_words_list, stop_words_list, prompt_template)

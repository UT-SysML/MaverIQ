...
import os
import sys
from MS_controller_baselines import inference_main, external_inputs, check_model_load, the_exit, set_inference_stop, set_req_id
...
def rename_output(args, client_id):
    ...
    new_file_name = f'{OUTPUTS_DIR}/{args.baseline}_{args.model_list}_None_0_{args.scale_factor}_None_{args.trace}_{args.slo}_{client_id}_output_records.csv'
...
def trace_replay(args, model_list, trace_list):
    ...
    save_model = True
    external_input = True
    print(f"Automaticly setting packing_threshold to default value - It will not be used")

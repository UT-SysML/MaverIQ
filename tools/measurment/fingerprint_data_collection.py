...
with open(f'{HOME_DIRECTORY}/outputs/profiling_data/fingerprint_data_collection.csv', mode='w', newline='') as file:
    ...
    for model_ in ['falcon-7b', 'falcon-40b', 'gptj-6b', 'llama-2-7b', 'llama-2-13b', 'llama-2-70b']:
        print(f'\n---------- {model_} ----------')
            ...
            for num_layers in [pp_deg_, pp_deg_+1]:
                ...
                if num_layers >= pp_deg_:
                    ...
                    for output_lenght_ in [10, 20]:
                        ...
                        writer.writerow([model_, tp_deg_, pp_deg_, pp_map_, dtype_, int8_kv_, output_lenght_, 1, latency, memory_pre, memory_after, time_generate_submodel, t_gen_graph, t_prof])

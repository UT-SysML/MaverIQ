...
with open(f'{HOME_DIRECTORY}/outputs/measurment_study/BS_scaling_data.csv', mode='w', newline='') as file:
    ...
    for model_ in ['llama-2-7b']:#['falcon-7b', 'falcon-40b', 'gptj-6b', 'llama-2-7b', 'llama-2-13b', 'llama-2-70b']:
        print(f'\n---------- {model_} ----------')
        trtllm_dir_ = model_.split('-')[0]
        model_dir_ = 'llama-2' if (trtllm_dir_== 'llama') else trtllm_dir_

        if model_ == 'falcon-7b': 
            configs_list = [(2, 4, 'int4', False)]
            num_layers = 32
        ...
        for (tp_deg_, pp_deg_, dtype_, int8_kv_) in configs_list:
            ...
            for batch_size_ in [1,2,4,8,16,32,64,128,256,512,1024]:
